...
export default function ResendVerificationPage() {
  ...
  const { register, handleSubmit, formState: { errors } } = useForm<ResendFormValues>({
  ...
  const onSubmit = async (data: ResendFormValues) => {
    setIsLoading(true);
    try {
      const response = await fetch('http://localhost:5000/api/v1/auth/resend-confirmation-email', {
        method: 'POST',
    ...
    <div className="min-h-screen flex items-center justify-center bg-background p-4">
      <div className="max-w-md w-full">
        ...
        >
          ...
          <div className="mb-10">
            <h1 className="text-3xl font-black tracking-tight mb-3">Resend <span className="text-primary">Verification</span></h1>
            <p className="text-muted-foreground font-medium">Enter your email and we&apos;ll send you a new confirmation link.</p>
          </div>
